<?php 
    ...
    if (isset($_SESSION[$lang]['peliculas_del_mes'])) {
        ...
        $bannerMovie = $bannerMovie['response'];
...
?>
<script>
    let moviesProfileUser = <?=json_encode($perfil->acf_fields->peliculas)?>;
    let bannerMovie = <?=json_encode($bannerMovie)?>;
</script>
<main data-ciclo_del_mes="<?= implode(',', $sdk->generalInfo->acf->ciclo_del_mes) ?>" data-peliculas_en_lo_mas_visto="<?= count($peliculasTopCombinadas) > 0 ? implode(',',$peliculasTopCombinadas) : implode(',',$sdk->generalInfo->acf->peliculas_en_lo_mas_visto)?>">
    <div class="banner">
        ...
        <div class="swiper swiperHomeApp">
            <!-- Additional required wrapper -->
            <div class="swiper-wrapper">
                <!-- Slides -->
                <?php for($i=0;$i<count($bannerMovie);$i++){ $post = $bannerMovie[$i]; ?>
                <div class="swiper-slide" data-movieid="<?=$post->id?>">
                    <picture>
            ...
            <div class="overlay">
                ...
                <div class="actions">
                    ...
                    ?>
                    <a href="<?=$lang?>/pelicula/<?=$sdk->get_alias($post->title->rendered)?>-<?=$post->id?>" class="btn btn-primary">Reproducir película</a>
                    <a href="javascript:;" onclick="getInfoMovie(<?=$post_json_escaped?>)" data-fancybox="" data-src="#dialog-content" class="btn btn-secondary">Mas información</a>
                </div>
            </div>
            <div class="video-actions">
               
                <button type="button" onclick="addMovieToFavorites(<?=$post_json_escaped?>)"><svg width="40" height="40" viewBox="0 0 40 40" fill="none"
    ...
    <section class="carousels container">
        <article id="colecciones"> 
            ...
            <div class="swiper swiperCarousel">
                ...
                <!-- Additional required wrapper -->
                 <?php $colls = $sdk->getCollections($perfil->acf_fields->colecciones_disponibles); ?>
                <div class="swiper-wrapper">
                    <?php 
                        for ($i=0; $i < count($colls["response"]); $i++) { 
                            $collection = $colls["response"][$i];
                    ?>
                     <div class="swiper-slide">
             <a href="<?=$lang?>/colecciones/<?=$sdk->get_alias($collection->title->rendered)?>-<?=$collection->id?>" >
                <?php if($collection->acf->image){?>
                    <img loading="lazy" class="lazyload" src="https://picsum.photos/20/20" data-src="<?=$collection->acf->image?>" alt="Logo Pelicula">
                <?php }?>
               <span class="title-movie"><?=$collection->title->rendered?></span>
             </a></div>
                    <?php 
                     }
                    ?>
