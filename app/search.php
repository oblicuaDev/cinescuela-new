<?php 
    ...
    // query($endpoint, $body = "", $method = "GET", $page = 1, $per_page = 50, $extra = [], $cache = true)
    // Realizar las consultas a la API
    $posts_posts = $sdk->query("posts", "", "GET", 1, 99, ['search'=> urlencode($_GET['search-input'])]);
    $posts_movies = $sdk->query("cinescuela-movies", "", "GET", 1, 99, ['search'=> urlencode($_GET['search-input'])]);
    $posts_ciclo = $sdk->query("cinescuela-ciclo", "", "GET", 1, 99, ['search'=> urlencode($_GET['search-input'])]);

    $posts_posts = $posts_posts['response'];
    $posts_movies = $posts_movies['response'];
    $posts_ciclos = $posts_ciclo['response'];

?>
<main data-searchval="<?=$_GET['search-input']?>" data-asignatura="<?=$_GET['subject']?>"
    data-clasificacion="<?=$_GET['age']?>">
    <section class="results container">
        <?=$title?>
        <ul class="movies-list-search ">
            <?php
            for ($i=0; $i < count($posts_movies); $i++) { 
                $movie = $posts_movies[$i];
            ?>
            <li>
                <a href="<?=$lang?>/pelicula/<?=$sdk->get_alias($movie->title->rendered)?>-<?=$movie->id?>">
                    ...
                    <img loading="lazy" class="lazyload movieLogo" src="<?=$movie->acf->logo_de_la_pelicula?>"
                        data-src="<?=$movie->acf->logo_de_la_pelicula?>" alt="Logo Pelicula">
                    <?php }else{ ?>
        ...
        <ul class="ciclos-list-search ">
            <?php
            for ($i=0; $i < count($posts_ciclos); $i++) { 
                $ciclo = $posts_ciclos[$i];
                
            ?>
            <li><a href="es/ciclos/<?=$sdk->get_alias($ciclo->title->rendered)?>-<?=$ciclo->id?>">
                    <div class="image"><img data-src="<?=$ciclo->acf->imagen_principal_el_ciclo?>" loading="lazy"
                            class="lazyload" src="<?=$ciclo->acf->imagen_principal_el_ciclo?>" alt="El buen comer">
                    </div>
                    <div class="info">
                        <h2><?=$ciclo->title->rendered?></h2><small><?=$ciclo->acf->mes_del_ciclo?>
                            <?=$ciclo->acf->ano_del_ciclo?></small>
                    </div>
                </a></li>
            <?php } ?>
        </ul>
        <div class="novedades-list ">
            <?php 
            for ($i=0; $i < count($posts_posts); $i++) { 
                $noticia =  $posts_posts[$i]; 
        ...
        ?>
            <a href="es/informacion/<?=$sdk->get_alias($noticia->title->rendered)?>-<?=$noticia->id?>">
                <div class="image">
                    <img data-src="<?=$noticia->acf->imagen?>" loading="lazy" class="lazyload"
                        src="<?=$noticia->acf->imagen?>">
                    <div class="badge">
                        <?=$icon?>
                        <span>
                            <?=$nameCat[0]?>
