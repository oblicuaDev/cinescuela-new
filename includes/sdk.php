...
class Cinescuela {
    ...
    public function query($endpoint, $body = "", $method = "GET", $page = 1, $per_page = 50, $extra = [], $cache = true){
        ...
        $url = urldecode($url);
         ...
         if ($response_headers !== false) {
             // Obtener el encabezado 'x-wp-total'
             $numPosts = isset($response_headers['x-wp-total']) ? $response_headers['x-wp-total']:1;
             $totalPages = isset($response_headers['x-wp-totalpages']) ? $response_headers['x-wp-totalpages']:1;
         } else {
             // Manejar la situación en caso de error
             echo "Error al obtener los encabezados de respuesta.";
         }
    ...
    function getNovedades($ids = "", $page = 1, $per_page = 3, $extra = ['field'=>'destacado', 'value'=> 1, 'categories'=> 9,'order'=> 'asc']){
